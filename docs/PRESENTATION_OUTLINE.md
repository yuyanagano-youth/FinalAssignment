...
**Q1: バックエンド完成後、フロント側で何か変更が必要？**

A: いいえ。`amhs-core.js` の `USE_MOCK_API = false` に変更するだけ。

**Q2: エラーハンドリングはどうする？**

A: 各 HTTP レスポンスの `errorCode` と `message` を確認。
詳細は `QUICK_REFERENCE.md` の「エラー設計」セクション。

**Q3: コンソール側が実装される前は？**

A: Main タブの「コンソール側ハートビートを1回疑似送信」ボタンで動作確認可能。

**Q4: 本番でも mock-server.js を含める？**

A: いいえ。`USE_MOCK_API = false` の場合、mock-server.js は呼ばれません。
ビルド時に削除するか、条件付きで exclude すればOK。

**Q5: sessionStorage に容量制限はある？**

A: ブラウザ環境で通常 5~10MB。本システムはテスト用なので問題なし。
本番で大量データを扱う場合は IndexedDB の検討も。

---

**以上です。何かご質問がありましたらお聞きします。**
